  ...
  {showMenu && (
    <div className="absolute left-0 bottom-full mb-2 ml-2 w-45 bg-white rounded-lg shadow-lg border z-50 ">
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-white border-t flex justify-around py-2 z-50">
        ...
        <Link to="/admin/rooms" className="flex flex-col items-center text-sm">
          <i className="fa-solid fa-people-roof"></i>
          <span>Phòng</span>
        </Link>
        <Link to="/admin/bookings" className="flex flex-col items-center text-sm">
          <i className="fa-solid fa-calendar-check"></i>
          <span>Đặt Phòng</span>
        </Link>
        <Link to="/admin/users" className="flex flex-col items-center text-sm">
          <i className="fa-solid fa-user-group"></i>
          <span>Người Dùng</span>
        </Link>
        <div className="flex flex-col items-center text-sm cursor-pointer" onClick={() => setShowMenu((prev) => !prev)}>
        ...
        {/* Menu popup mobile */}
        {showMenu && (
          <div className="absolute bottom-14 right-4 bg-white shadow rounded w-48 z-50 p-2">
            <button className="block w-full text-left px-4 py-2 hover:bg-gray-100 rounded" onClick={handleEditProfile}>
              Chỉnh sửa trang cá nhân
            </button>
            <button className="block w-full text-left px-4 py-2 hover:bg-gray-100 rounded" onClick={handleLogout}>
              Đăng xuất
            </button>
            <button className="block w-full text-left px-4 py-2 hover:bg-gray-100 rounded" onClick={handleOther}>
              Khác
            </button>
          </div>
        )}
